...
В `examples/` живут четырнадцать работающих приложений (в репозитории, не в
...
web-слой на каноническом контракте `ctxloom.chat` + `ctxloom.web` — каждое
...
**Что показывает:** адаптивный планировщик (`ctxloom.scheduler`) в деле —
